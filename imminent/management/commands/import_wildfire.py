...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for data in response_data:
            ...
            uuids = Pdc.objects.filter(status=Pdc.Status.ACTIVE, hazard_type=HazardType.WILDFIRE).values_list('uuid', 'hazard_type', 'pdc_updated_at')
            ...
            # wildfire polygon
            uuids = Pdc.objects.filter(status=Pdc.Status.ACTIVE, hazard_type=HazardType.WILDFIRE).values_list('uuid', flat=True)
